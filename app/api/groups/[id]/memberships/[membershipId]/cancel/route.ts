...
/**
 * PATCH /api/groups/[id]/memberships/[membershipId]/cancel
 *
 * Cancels a pending group membership invitation.
 * Only the group owner can cancel pending invitations sent to other users.
 * Changes the membership status from PENDING to CANCELED.
 * Returns the updated group membership with all related details.
 *
 * Return statuses:
 * - 200 OK : Group membership invitation successfully canceled.
 * - 403 Forbidden : User is not the owner of the group.
 * - 404 Not Found : Group or group membership does not exist.
 * - 409 Conflict : Membership is not in a pending state.
...
export const PATCH = withAuth(
  async (
    ...
    const logger = createLogger(
      'api/groups/[id]/memberships/[membershipId]/cancel:patch',
      request,
    );
